...
function findEmployeeBySpecialization(employeeSpec) {
    const foundSpec = employees.find(employee => employee.specialization === 'Javascript');
    if (foundSpec) {
    document.getElementById('employeesDetails').innerHTML =`<p>${foundSpec.id}: ${foundSpec.name}  - ${foundSpec.department} - ${foundSpec.specialization}</p>`;
      ...
      document.getElementById('employeesDetails').innerHTML = 'aucun employé n\'a été trouvé avec cette specialisation';
